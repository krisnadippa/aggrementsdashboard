...
export async function POST(req: NextRequest) {
  try {
    ...
    if (idParam) {
      ...
      // 2. Fetch existing contract from DB first to prevent parameter tampering and check existence
      const rows = await sql`SELECT data FROM rental_contracts WHERE id = ${idParam}`;
      if (rows.length === 0) {
      ...
      // 3. Conditional update validation: Admin vs Customer
      let updatedData;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await sql`SELECT data FROM rental_contracts WHERE id = ${id}`;
    if (rows.length === 0) {
