    ...
    <div className="page-inner">
      ...
      <div className="invoice-page-actions no-print" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          <Link href={record?.formData?.dashboardVersion === 'dashboard1' ? `/?edit=${id}` : `/dashboard2?edit=${id}`} className="btn btn-outline" style={{ borderColor: 'var(--accent)', color: 'var(--accent)' }} id="edit-invoice-detail-btn">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          {formState?.phone && (
            <a
              href={`https://wa.me/${formatWhatsAppNumber(formState.phone)}?text=${encodeURIComponent(`Halo *${formState.renterName || 'Pelanggan'}*,\n\nBerikut adalah link dokumen sewa kendaraan *${formState.vehicleName || ''}* (${formState.policeNumber || ''}):\n${typeof window !== 'undefined' ? window.location.href : ''}\n\nTerima kasih!`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="btn btn-outline"
              style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', borderColor: '#25D366', color: '#25D366' }}
              title="Kirim link invoice ke WhatsApp penyewa"
            >
        ...
        <div className="interactive-tab-view no-print" style={{ maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <section className="form-card-section" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem', fontSize: '0.875rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px dashed var(--border)', paddingBottom: '0.375rem' }}>
                <span style={{ color: 'var(--text-secondary)' }}>No. Telepon (WhatsApp)</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{formState.phone || '—'}</span>
                  {formState.phone && (
                    <a
                      href={`https://wa.me/${formatWhatsAppNumber(formState.phone)}`}
                      target="_blank"
                      rel="noopener noreferrer"
