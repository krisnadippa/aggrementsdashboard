...
export default function InvoiceForm2({ onSubmit, prefillData }: InvoiceFormProps) {
  ...
  // Load prefill data from parent (e.g. signed data fetched from API by dashboard2)
  useEffect(() => {
    if (prefillData) {
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      balance: balanceDue >= 0 ? balanceDue : 0,
    }));
  }, [
    form.startDate,
    form.startTime,
  ...
  return (
    <form className="invoice-form" onSubmit={handleSubmit} id="rental-form">
      ...
      <div className="form-header-bar no-print">
        <div>
          <h1 className="page-title">Buat Perjanjian Sewa (Link Mandiri)</h1>
          <p className="page-subtitle">Pilih metode bagikan link untuk dikirimkan dan ditandatangani mandiri oleh penyewa.</p>
        </div>
      </div>
        ...
        <div className="form-column-right">
          ...
          <section className="form-card-section">
            ...
            <div className="signature-grid">
              ...
              />
              <SignaturePad
                label="Tanda Tangan Penyewa (Dapat dikosongkan untuk dikirim)"
          ...
          {/* Share Signature and Submission Buttons */}
          <div className="form-actions-bottom no-print" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <button type="submit" className="btn btn-primary btn-lg" id="generate-invoice-bottom-btn" style={{ width: '100%' }}>
              <span>Simpan & Buat Invoice Final</span>
            </button>
          </div>
